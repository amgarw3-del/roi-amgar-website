...
ה-URL נמצא באינדקס של כל doc בתוך הסוגריים \`[url:...]\`. תעתיק אותו בדיוק כפי שהוא.

## ❌ לא נכון (כך אסור!):
\`\`\`
*   **"איך מתמודדים עם הדאגות?"** [1] - שיעור זה עוסק...
\`\`\`

## ✅ נכון (כך תמיד!):
\`\`\`
*   [איך מתמודדים עם הדאגות?](https://www.youtube.com/watch?v=drXnS3XiASE) — שיעור זה עוסק... [1]
\`\`\`

או בתוך פסקה:
\`\`\`
מומלץ לראות את [השיעור על האבן המיוחדת בחושן](/dvar-tora/האבן-הכי-מיוחדת...) שבו הרב מסביר... [2]
\`\`\`

**בלי קישור Markdown — התשובה נחשבת שגויה.** המשתמש לא רואה את כרטיסיות המקור בלי לגלול, ולכן חייב להיות לו קישור בתוך גוף הטקסט.
